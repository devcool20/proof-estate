    ...
    <header className="flex items-center justify-between whitespace-nowrap border-b border-border-light bg-white px-6 md:px-10 py-3 sticky top-0 z-[60]">
      <div className="flex items-center gap-6 md:gap-8">
        ...
        <div className="hidden lg:flex items-center gap-1 bg-background-light p-1 rounded-lg border border-border-light">
          ...
          {user?.role === "owner" && (
            <>
              <NavLink href="/properties" active={pathname === "/properties"} label="My Properties" />
              <NavLink href="/verify" active={pathname === "/verify"} label="Submit Asset" />
            </>
          )}

          {user?.role === "investor" && (
            <NavLink href="/explore" active={pathname === "/explore"} label="Marketplace" />
          )}

          {user?.role === "admin" && (
            <NavLink href="/admin" active={pathname === "/admin"} label="Admin Dashboard" />
          )}
        </div>
      </div>
      <div className="flex items-center gap-4">
        ...
        <Show when="signed-in">
          ...
          <UserButton 
            appearance={{
              elements: { userButtonAvatarBox: "h-10 w-10 border border-slate-200 cursor-pointer shadow-sm hover:ring-2 hover:ring-primary/20 transition-all rounded-full" }
            }}
          />
        </Show>
        ...
        <Show when="signed-out">
          <SignInButton mode="modal">
            <button className="h-10 px-5 font-bold rounded-lg bg-primary text-white hover:bg-slate-800 transition-colors shadow-sm">
              Sign In
            </button>
          </SignInButton>
        </Show>
